			...
			if multiphaseEvaluation && !r.HasChain && r.ParentID_ == 0 {
				...
				for _, matched := range tx.matchedRules {
					if matched.Rule().ID() == r.ID_ {
						// Rule already triggered at an earlier phase, so we avoid double evaluation
						tx.WAF.Logger.Debug("[%s] [%d] Skipping evaluation of rule %d, already matched", tx.id, rid, r.ID_)
